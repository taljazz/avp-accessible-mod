...
/* Accessibility: Get all visible mission hints/objectives as text */
int GetMissionObjectivesText(char* buffer, int bufferSize)
{
	...
	/* Iterate through visible mission hints */
	const List<MissionHint*>& hints = MissionHint::GetAll();

	for (LIF<MissionHint*> hi((List<MissionHint*>*)&hints); !hi.done(); hi.next())
	{
		MissionHint* hint = hi();
		if (hint && hint->bVisible())
		{
			SCString* desc = hint->GetDesc();
			if (desc)
			{
				const char* text = desc->pProjCh();
				if (text && text[0])
				{
					objectiveCount++;
					int len = strlen(text);

					/* Add objective number prefix */
					char prefix[32];
					snprintf(prefix, sizeof(prefix), "Objective %d: ", objectiveCount);
					int prefixLen = strlen(prefix);

					if (totalLen + prefixLen + len + 2 < bufferSize)
					{
						strcat(buffer, prefix);
						strcat(buffer, text);
						strcat(buffer, ". ");
						totalLen += prefixLen + len + 2;
					}
				}
				desc->R_Release();
			}
	...
	if (objectiveCount == 0)
	{
		strncpy(buffer, "No active objectives.", bufferSize - 1);
