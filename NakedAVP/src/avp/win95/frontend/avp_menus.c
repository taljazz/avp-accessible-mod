...
static void Accessibility_AnnounceMenuSelection(void)
{
    ...
    {
        int profileIndex = *(elementPtr->c.SliderValuePtr);
        AVP_USER_PROFILE *profilePtr = GetFirstUserProfile();

        for (int i = 0; i < profileIndex && profilePtr; i++)
        {
    ...
    /* Special handling for episode select menus */
    if (AvPMenus.CurrentMenu == AVPMENU_MARINELEVELS ||
        AvPMenus.CurrentMenu == AVPMENU_PREDATORLEVELS ||
        AvPMenus.CurrentMenu == AVPMENU_ALIENLEVELS)
    {
        int episodeIndex = *(elementPtr->c.SliderValuePtr);
        /* Episode names are indexed from TextDescription */
        const char *episodeName = GetTextString(elementPtr->a.TextDescription + episodeIndex);
    ...
    switch(elementPtr->ElementID) {
        ...
        {
            /* Text slider - announce label and current value */
            label = GetTextString(elementPtr->a.TextDescription);
            value = GetTextString(elementPtr->d.FirstTextSliderString + *(elementPtr->c.SliderValuePtr));
            if (label && value)
            {
                snprintf(announceBuffer, sizeof(announceBuffer), "%s: %s", label, value);
        ...
        {
            /* Text slider with pointer - announce label and current value */
            label = GetTextString(elementPtr->a.TextDescription);
            if (elementPtr->d.TextSliderStringPointer)
            {
                value = elementPtr->d.TextSliderStringPointer[*(elementPtr->c.SliderValuePtr)];
            }
            if (label && value)
            {
                snprintf(announceBuffer, sizeof(announceBuffer), "%s: %s", label, value);
        ...
        {
            /* Numeric slider - announce label and percentage */
            label = GetTextString(elementPtr->a.TextDescription);
            int currentVal = *(elementPtr->c.SliderValuePtr);
            int maxVal = elementPtr->b.MaxSliderValue;
        ...
        {
            /* Toggle - announce label and on/off state */
            label = GetTextString(elementPtr->a.TextDescription);
        ...
        {
            /* Number field - announce label and value */
            label = GetTextString(elementPtr->a.TextDescription);
            int numVal = *(elementPtr->c.NumberPtr);
            const char *units = GetTextString(elementPtr->d.NumberFieldUnitsString);
            if (units && units[0])
            {
                snprintf(announceBuffer, sizeof(announceBuffer), "%s: %d %s", label ? label : "Value", numVal, units);
            }
            else
            {
        ...
        {
            /* Text field - announce the field content or label */
            label = GetTextString(elementPtr->a.TextDescription);
            value = elementPtr->c.TextPtr;
        ...
        {
            /* Save/Load game slots - announce slot number */
            int slotNum = AvPMenus.CurrentlySelectedElement + 1;
        ...
        {
            /* Difficulty selection - announce "Difficulty: Easy/Medium/Hard" */
            label = GetTextString(elementPtr->a.TextDescription);
            if (label && label[0])
            {
        ...
        {
            /* Menu navigation buttons - announce with context */
            label = GetTextString(elementPtr->a.TextDescription);
            if (label && label[0])
            {
                TTS_Speak(label);
        ...
        default:
        {
            /* Most elements use TextDescription */
            label = GetTextString(elementPtr->a.TextDescription);
            if (label && label[0])
            {
                TTS_Speak(label);
...
static void SetupNewMenu(enum AVPMENU_ID menuID)
{
	...
	/* Accessibility: Announce menu title, then first menu item when entering a new menu */
	{
		const char* menuTitle = GetTextString(AvPMenusData[menuID].MenuTitle);
		if (menuTitle && menuTitle[0])
		{
			/* Announce menu title first */
			TTS_Speak(menuTitle);
		}
